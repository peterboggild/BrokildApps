...
param([switch]$NoBuild, [string]$Extra = "")
$root = "$PSScriptRoot\.."
if (-not $NoBuild) {
    if (-not (Test-Path "$root\test\build\CMakeCache.txt")) {
        cmake -S "$root\test" -B "$root\test\build" -A x64 | Out-Null
    }
    cmake --build "$root\test\build" --config Release --target ttytest 2>&1 | Select-String -Pattern "error|warning C4|ttytest.vcxproj ->" | ForEach-Object { $_.Line }
    if ($LASTEXITCODE -ne 0) { Write-Output "BUILD FAILED"; exit 1 }
}
$exe = "$root\test\build\Release\ttytest.exe"
if (-not (Test-Path $exe)) { Write-Output "no exe"; exit 1 }
$rand = New-Object System.Random
for ($i = 0; $i -lt 6; $i++) {
    $copy = "$root\test\build\Release\ttytest_run$i.exe"
    ...
    try {
        if ($Extra -ne "") { & $copy $Extra.Split(" ") } else { & $copy }
        $code = $LASTEXITCODE

...
function readWav(file) {
  ...
  // 16- or 24-bit PCM (the render tool writes 24)
  const bps = fmt.bits === 24 ? 3 : 2, frame = bps * fmt.ch;
  const n = Math.floor(data.length / frame);
  const L = new Float32Array(n), R = new Float32Array(n);
  const rd = (o) => bps === 3 ? (data.readIntLE(o, 3) / 8388608) : (data.readInt16LE(o) / 32768);
  for (let i = 0; i < n; i++) { L[i] = rd(i * frame); R[i] = rd(i * frame + bps); }

...
param(
  ...
  [string]$Exe  = "$PSScriptRoot\..\build\ThirtyThousandYears_artefacts\Release\Standalone\Thirty Thousand Years.exe",
  [switch]$KeepOpen
)

Get-Process msedgewebview2 -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
Get-Process "Thirty Thousand Years" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue

    ...
    : rack (r), apvts (s)
{
    ...
    styleHead (foot, juce::String ("BWFX ") + bwfx::Rack::version()
                     + "   -   the rack runs on the MAIN MIX, after the eight slots; a slot sent"
                       " to its OWN output goes round it", 10.0f, kSmoke);
    ...
    /*  SPECTRA writes a modulation BUS that the host engine has to consume,
        and Beetmachine does not consume it — a drum has no voices to detune. Saying so is
    ...
    styleHead (busNote,
               rack.isWorldModConsumed()
                   ...
                   : "the characters below that carry AUDIO work here; the ones that only"
                     " write the modulation bus do not, because a drum kit has no synth"
                     " voices to bend",
               10.0f, kSmoke);

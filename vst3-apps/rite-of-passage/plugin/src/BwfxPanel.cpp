    ...
    : rack (r), apvts (s)
{
    ...
    styleHead (foot, juce::String ("BWFX ") + bwfx::Rack::version()
                     + "   -   the rack runs AFTER the whole rite, so it colours the"
                       " transition rather than taking part in it", 10.0f, kSmoke);
    ...
    /*  SPECTRA writes a modulation BUS that the host engine has to consume,
        and Rite does not consume it — it has no voices to detune. Saying so is
        better than showing six characters that quietly do nothing, which is
        what the fragment's own "arriving" plate exists to avoid. */
    styleHead (busNote,
               rack.isWorldModConsumed()
                   ...
                   : "the characters below that carry AUDIO work here; the ones that only"
                     " write the modulation bus do not, because a transition processor"
                     " has no voices to bend",
               10.0f, kSmoke);

...
//
//   sttest --levels > levels.txt
//   node tools/level-presets.js levels.txt
//
// Target: -15 dB RMS over the first 150 ms (the part of a snare the ear
...
const TARGET_RMS = -15.0, MAX_PEAK = -1.0;

const file = path.resolve(__dirname, "../engine/st_presets.cpp");
...
for (const ln of lines) {
    ...
    if (v === "nullptr") {
        ...
        s = s.replace("#undef ST_VALUES", 'ST_VALUES (vInit, {"level",0})\n\n#undef ST_VALUES');
        s = s.replace('"A plain electronic kick to start from.", nullptr, 0 }', '"A plain electronic kick to start from.", vInit, ST_N (vInit) }');
        v = table[name] = "vInit";
    }
    const delta = Math.min(TARGET_RMS - rms, MAX_PEAK - peak);
    const re = new RegExp("ST_VALUES \\(" + v + ",\\s*");

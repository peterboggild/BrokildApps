# Cut the Snare Tactics download and PROVE it: the plug-in and the standalone
...
param([string] $BuildDir = "C:\Users\peter\b\_build\SnareTactics\plugin")
$ErrorActionPreference = "Stop"

$site   = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)          # vst3-apps\snare-tactics
$rel    = Join-Path $BuildDir "SnareTactics_artefacts\Release"
$vst3   = Join-Path $rel "VST3\Snare Tactics.vst3"
$exe    = Join-Path $rel "Standalone\Snare Tactics.exe"
$pdf    = Join-Path $site "Snare-Tactics-Manual.pdf"
...
$id = [regex]::Match($cml, 'ST_BUILD_ID "([0-9.]+)"').Groups[1].Value

$stageRoot = Join-Path $env:TEMP "snare-tactics-dist"
if (Test-Path $stageRoot) { Remove-Item $stageRoot -Recurse -Force }
$stage = Join-Path $stageRoot "Snare-Tactics-VST3-win64"
New-Item -ItemType Directory -Force $stage | Out-Null
Copy-Item $vst3 (Join-Path $stage "Snare Tactics.vst3") -Recurse
...
$zip = Join-Path $site "Snare-Tactics-VST3-win64.zip"
if (Test-Path $zip) { Remove-Item $zip -Force }
Compress-Archive -Path $stage -DestinationPath $zip

# ---- verify from the archive itself -------------------------------------------
$chk = Join-Path $env:TEMP "snare-tactics-zipcheck"
if (Test-Path $chk) { Remove-Item $chk -Recurse -Force }
Expand-Archive $zip $chk
$dll = Get-ChildItem $chk -Recurse -Filter "Snare Tactics.vst3" -File | Select-Object -First 1
$sx  = Get-ChildItem $chk -Recurse -Filter "Snare Tactics.exe" -File | Select-Object -First 1
$okDll = (Get-FileHash $dll.FullName).Hash -eq (Get-FileHash (Join-Path $vst3 "Contents\x86_64-win\Snare Tactics.vst3")).Hash

...
## 7. The daughters' type in Beetmachine's style (Peter, 2026-09-26) - SHIPPED 260926.4 (see below)
...
- **Beetmachine 260926.7 + Kickstart/Snare Tactics/Hats Off 260926.4** (2026-09-26): (a) 24 kits - fourteen
  new styles; a kit slot can adjust its drum's controls after the preset (`tweaks`, real units, by id) and a
  kit can carry a BWFX rack; loading a kit now SETS the rack (empty for kits without one). (b) The rack is the
  standard web rack (bwfx-rack.js in WebView2), Black Rider's. (c) The family's type: `machineart::stencil` /
  `machineart::mono` (Arial Black / Consolas bold) in all four panels - buttons, value boxes, menus and the
  toggle included. Kit lists in every doc are generated by `beetmachine/plugin/tools/write-kit-docs.js`.
